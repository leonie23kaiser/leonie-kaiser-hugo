...
- Compare only findings tied to the public crawl surface: `robots.txt`, `sitemap.xml`, visible navigation, and public pages reachable from those entry points.
- Do not carry forward or compare findings derived solely from hidden routes, non-public Hugo structure, or guessed URLs.
- Treat local preview host/port differences as audit-environment notes rather than SEO deltas unless the same mismatch is visible on a public deployment or the user explicitly asked to debug the local setup.
